...
(() => {
    ...
    const setFontPx = (node, px) => {
        node.style.fontSize = clamp(px, 8, 300) + "px";
        if (selected === node && fontSize) fontSize.value = Math.round(clamp(px, 8, 300));
    };
    const setRotation = (node, deg) => {
        const v = String(deg).includes("deg") ? String(deg) : deg + "deg";
        node.style.rotate = v;
        node.style.transform = `rotate(${v.replace("deg", "")}deg)`;
    };
    ...
    const select = (node) => {
        ...
        if (selected.dataset.type === "text") {
            ...
            if (color) {
                ...
                if (m) {
                    const [, r, g, b, a] = m;
                    color.value = "#" + [r, g, b].map(n => (+n).toString(16).padStart(2, "0")).join("");
                    textAlpha && (textAlpha.value = (a ?? 1));
                }
            }
            if (bg) {
                ...
                if (m) {
                    const [, r, g, b, a] = m;
                    bg.value = "#" + [r, g, b].map(n => (+n).toString(16).padStart(2, "0")).join("");
                    bgAlpha && (bgAlpha.value = (a ?? 1));
                } else {
                    bg.value = "#000000";
                    bgAlpha && (bgAlpha.value = "0");
                }
            }
    ...
    $(".add-image")?.addEventListener("click", () => {
        ...
        input.addEventListener("change", async () => {
            ...
            if (!f) return;
            createImageNode(URL.createObjectURL(f));
        });
        input.click();
    });
    ...
    inputText?.addEventListener("input", () => {
        if (selected?.dataset.type === "text") selected.textContent = inputText.value;
    });
    fontFamily?.addEventListener("change", () => {
        if (selected?.dataset.type === "text") selected.style.fontFamily = fontFamily.value;
    });
    fontSize?.addEventListener("input", () => {
        if (selected?.dataset.type === "text") setFontPx(selected, parseFloat(fontSize.value || 24));
    });
    const updateTextColor = () => {
        if (selected?.dataset.type === "text") applyCssColor(selected, "color", color.value, textAlpha?.value ?? 1);
    };
    const updateBgColor = () => {
        if (selected?.dataset.type === "text") applyCssColor(selected, "backgroundColor", bg.value, bgAlpha?.value ?? 0);
    };
    ...
    rotation?.addEventListener("input", () => {
        if (selected) setRotation(selected, rotation.value);
    });
    opacity?.addEventListener("input", () => {
        if (selected) selected.style.opacity = opacity.value;
    });
    ...
    boldBtn?.addEventListener("click", () => {
        if (selected?.dataset.type === "text") {
            selected.style.fontWeight = /^(bold|700)$/i.test(selected.style.fontWeight) ? "400" : "700";
            updateToggleState();
        }
    });
    ...
    forwardBtn?.addEventListener("click", () => {
        if (selected) selected.style.zIndex = String(++zCounter);
    });
    backwardBtn?.addEventListener("click", () => {
        if (selected) selected.style.zIndex = String(Math.max(1, (+selected.style.zIndex || 1) - 1));
    });
    deleteBtn?.addEventListener("click", () => {
        if (!selected) return;
        const rm = selected;
        select(null);
        if (rm.dataset.type === "text") ro.unobserve(rm);
        rm.remove();
    });
    ...
    const serializeOverlay = async () => {
        ...
        const stageInfo = {
            ...
            height: stage?.clientHeight ?? null,
    ...
    const clearNodes = () => {
        if (!layer) return;
        for (const n of Array.from(layer.querySelectorAll(".text-node, .image-node"))) {
    ...
    const createNodeFromData = (d) => {
        ...
        if (d.type === "image") {
            ...
            Object.assign(node.style, {
                position: "absolute",
                left: (typeof d.left === "number" ? d.left + "px" : d.left || "10%"),
                top: (typeof d.top === "number" ? d.top + "px" : d.top || "10%"),
                resize: "both",
                overflow: "hidden",
                cursor: "move"
            });
    ...
    const attachToGlobalFileInput = () => {
        ...
        fileInput.addEventListener("change", async () => {
            ...
            if (f.type === "application/json" || f.name.toLowerCase().endsWith(".json")) {
                try {
                    await importOverlayFromObject(JSON.parse(await f.text()));
                } catch {
                    console.warn("Invalid overlay JSON");
                }
            } else if (f.type.startsWith("image/")) {
                const fr = new FileReader();
                fr.onload = () => {
                    const imgEl = $("#overlay-image");
                    imgEl ? (imgEl.src = fr.result) : createImageNode(fr.result);
                };
                fr.readAsDataURL(f);
            }
        });
    ...
    const attachKeyboardShortcuts = () => {
        document.addEventListener("keydown", async (e) => {
            ...
            switch (e.key) {
                ...
                case "ArrowDown":
                    if (mod && selected) {
                        selected.style.zIndex = String(Math.max(1, (+selected.style.zIndex || 1) - 1));
                        e.preventDefault();
                        return;
                    }
                    if (selected) {
            ...
            if (k === "d") {                                                                                         // Alt+D duplicate
                ...
                const nn = createNodeFromData(data);
                if (nn) select(nn);
                return;
            }
            if (k === "b") {
    ...
    (() => {
        ...
        layer.addEventListener("mouseleave", () => {
            if (!resizingNode && !drag) layer.style.cursor = "default";
        });
